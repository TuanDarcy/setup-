...
echo.
echo [2/6] Setting up SET RAM (monitor_ram)...
...
if exist "!VOLTX_ZIP!" (
    ...
    if exist "!VOLTX_HEADLESS!" (
        :: Write voltUser / voltPass into config.json
...
echo.
echo [4/6] Downloading volt.exe...

set "VOLT_EXE=%DESKTOP%\volt.exe"
if exist "!VOLT_EXE!" del /f /q "!VOLT_EXE!" >nul 2>&1
...
echo.
echo [5/6] Downloading 1.1.1.1 WARP...

set "WARP_EXE=%DESKTOP%\Cloudflare_WARP.exe"
if exist "!WARP_EXE!" del /f /q "!WARP_EXE!" >nul 2>&1
...
if exist "!WARP_EXE!" (
    ...
    echo [!] 1.1.1.1 download FAILED - you can install manually later
)
